...
function orderSelected() {
	...
	// 동적으로 폼(Form) 생성하여 POST 전송
	var form = $('<form action="/order/form" method="post"></form>');

	// 선택된 카트 번호들을 하나씩 hidden input으로 추가
	for (var i = 0; i < selectedCartNos.length; i++) {
		form.append($('<input type="hidden" name="cartNos" value="' + selectedCartNos[i] + '">'));
	}
